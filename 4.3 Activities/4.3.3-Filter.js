...
const toDosSix = data.filter(task => task.userId === 6);
console.log(toDosSix);
const userSixTasks = data.filter(task => (task.userId === 6 && task.completed === true));
console.log(userSixTasks);


 // filter for tasks with a title less than 24 characters long
const shortTitle = data.filter(task => task.title.length < 24);
