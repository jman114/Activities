 ...
 const data = [
  ...
  const coords = data.map(obj => [obj.address.geo.lat, obj.address.geo.lng]);

  console.log(coords);


  // create an array of strings that combine the company's catchPhrase and bs
const combo = data.map(obj => [obj.company.catchPhrase, obj.company.bs]);

console.log(combo);

const phrase = data.map(obj => {
    return `${obj.company.catchPhrase} ${obj.company.bs}`
});
console.log(phrase);